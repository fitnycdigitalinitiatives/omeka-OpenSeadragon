    ...
    public function openseadragon($item)
    {
      if ($record_id = metadata($item, array('Item Type Metadata', 'Record ID'), array('all' => true))) {
        ...
        for($x = 0; $x < $arrlength; $x++) {
      		...
      		if (($record_name = metadata($item, array('Item Type Metadata', 'Record Name'), array('index' => $x, 'all' => true))) && ($record_id = metadata($item, array('Item Type Metadata', 'Record ID'), array('index' => $x, 'all' => true))) && ($width = metadata($item, array('Item Type Metadata', 'Width'), array('index' => $x, 'all' => true))) && ($height = metadata($item, array('Item Type Metadata', 'Height'), array('index' => $x, 'all' => true)))) {
            echo $record_id;
